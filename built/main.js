...
function uploadEmoji(teamName, emojiName, emojiBlob) {
    var teamUrl = "https://".concat(teamName, ".slack.com");
    var iconUrl = URL.createObjectURL(emojiBlob);
    if (teamName === null) {
        alert('Oops. No Slack team was entered.');
        chrome.runtime.openOptionsPage();
        return;
    ...
    chrome.notifications.create(undefined, {
        type: "basic",
        title: "Uploading",
        message: "Adding :".concat(emojiName, ":..."),
        iconUrl: iconUrl,
    });
    fetch("".concat(teamUrl, "/customize/emoji"))
        ...
        try {
            ...
            if (err.message === "Cannot read property '1' of null") {
                ...
                throw { message: "Please log in to ".concat(teamName), type: 'not-logged-in' };
            }
        }
        return apiToken;
    })
        ...
        fetch("".concat(teamUrl, "/api/emoji.add"), {
            method: 'POST',
            body: formData
        })
            ...
            if (!json['ok']) {
                ...
                var reason = errorReasons[json['error']] || json['error'];
                throw { message: reason };
            }
            chrome.notifications.create(undefined, {
                type: 'basic',
                title: 'Success!',
                message: ":".concat(emojiName, ": added to ").concat(teamName, "!"),
                iconUrl: iconUrl,
            });
        })
        ...
        msg = "Failed to add :".concat(emojiName, ": \n").concat(err.message);
